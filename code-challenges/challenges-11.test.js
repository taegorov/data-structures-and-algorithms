...
//   return [`<li>${objArray[0][0]}: ${objArray[0][1]}</li>, <li>${objArray[1][0]}: ${objArray[1][1]}</li>`]
// }
...
const count = (target, input) => {
  ...
  let value = newArray.filter(function (number) {
...
const totalSum = (input) => {
  let newArray = input.flat();
  let answer = newArray.reduce((acc, cur) => {
    acc = acc + cur;
    return acc;
  })
  return answer;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let firstArray = input.map(value => value.filter(num => !(num % 5)));
  let secondArray = firstArray.map(value => value.filter(num => typeof num !== 'string'));

  let solution = secondArray.map(value => value.map(num => Math.pow(2, num)));
  return solution;
  // onlyNumbers = numbersDivisibleByFive.filter(n => typeof num !=)
};
